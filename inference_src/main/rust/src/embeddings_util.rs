...
pub fn read_movie_embeddings(movie_embeddings_uri : &String) -> (Vec<f32>, usize, usize) {
    ...
    let file = File::open(movie_embeddings_uri).unwrap();
    ...
    let mut stacked_embeddings: Vec<f32> = Vec::with_capacity(num_movies * embed_len);
    for record in records {
        stacked_embeddings.extend(record.embedding);
    }

    (stacked_embeddings, num_movies, embed_len)
