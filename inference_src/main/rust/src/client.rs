...
impl RankerModelClient {
    ...
    pub async fn get_candidate_ranks(&self, padded_super_graph: JraphGraph, embed_len : usize) -> Result<(Vec<f32>), Box<dyn Error>> {
        ...
        let inputs : HashMap<String, TensorProto> = build_graph_ranker_proto_inputs(padded_super_graph, embed_len);
        ...
        let predict_req = tonic::Request::new(PredictRequest {
...
pub fn build_query_model_inputs(req: UserRequest) -> HashMap<String, TensorProto> {
    ...
    inputs.insert("timestamp".into(), vec![req.timestamp].into_tensor());

    inputs
}

pub fn build_graph_ranker_proto_inputs(padded_super_graph: JraphGraph, embed_len : usize) -> HashMap<String, TensorProto> {
    /*
    //MAX_GRAPHS:
