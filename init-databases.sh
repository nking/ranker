...
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "postgres" <<-EOSQL
    ...
    SELECT 'CREATE DATABASE vizier_db' WHERE NOT EXISTS (SELECT FROM pg_database WHERE datname = 'vizier_db')\gexec
    GRANT ALL PRIVILEGES ON DATABASE mlflow_db TO "$POSTGRES_USER";
    GRANT ALL PRIVILEGES ON DATABASE vizier_db TO "$POSTGRES_USER";
EOSQL
