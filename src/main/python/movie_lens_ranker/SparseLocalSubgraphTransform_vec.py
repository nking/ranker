...
class SparseLocalSubgraphTransform(pgrain.MapTransform):
    ...
    def map(self, batch:Dict[str, np.ndarray]) -> List[jraph.GraphsTuple]:
        ...
        for i in range(len(batch['user_id'])):
            ...
            edge_features[:n_real_history] = batch["rating"][i][:n_real_history]
            ...
            node_ids = jnp.concatenate([
                jnp.array([batch["user_id"][i]], dtype=int),
                jnp.array(batch["history_movie_ids"][i][:n_real_history], dtype=int),
                jnp.array(batch["candidate_ids"][i], dtype=int)
            ], dtype=int)
