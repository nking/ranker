...
def train_step(model: GraphRanker, padded_graph: jraph.GraphsTuple,
    ...
    def loss_fn(model, padded_graph) -> float:
        ...
        #this only masks out dummy graphs, not elements such as dummy nodes within real graphs:
        #mask = jraph.get_graph_padding_mask(padded_graph)
        
        # Rax Ranking Loss & Metrics
        # Rax is ignores entries where master_mask is False
        per_row_loss = rax.softmax_loss(
            scores=scores_2d,
            labels=labels_2d,
            where=main_mask,
            reduce_fn=None
        ...
        # 3. Mask the loss: set NaN or Padding-loss to 0.0
        # jnp.nan_to_num catches the NaN generated by the dummy graph
        safe_loss = jnp.nan_to_num(per_row_loss, nan=0.0)
        masked_loss = jnp.where(is_real_graph, safe_loss, 0.0)
        
        # 4. Manual Mean
        loss = jnp.sum(masked_loss) / jnp.maximum(jnp.sum(is_real_graph), 1.0)
        
        jax.debug.print("loss={loss}", loss=loss, ordered=True)
...
def eval_step(model: GraphRanker, padded_graph: jraph.GraphsTuple,
    ...
    def loss_fn(model, padded_graph) -> Tuple[Array, Dict[str, Array]]:
        ...
        mrr = rax.mrr_metric(
            scores_2d, labels_2d, where=main_mask, topn=20, reduce_fn=jnp.mean)
...
def train_fn(model, num_epochs: int, train_dataloader: grain.DataLoader,
    ...
    for epoch in range(num_epochs):
        ...
        jax.debug.print("Epoch {epoch}: Train Loss {avg_train_loss:.4f}", avg_train_loss=avg_train_loss)
